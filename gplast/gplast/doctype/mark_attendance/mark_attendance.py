...
# 	emps = frappe.get_all("Employee",{'status':'Active','date_of_joining':['<=',from_date]})
# 	for emp in emps:
...
def mark_permission(from_date):
	...
	WHERE permission_date = '%s' """%from_date,as_dict=True)
	for pr in pr_list:
			...
			if att.in_time and att.out_time:
				...
				if pr.session == 'First Half':
					if pr.shift == 'A':
						...
						shift_in = datetime.strptime('16:00', '%H:%M')
						diff = time_diff_in_hours(in_t,shift_in)
					elif pr.shift == 'C':
						shift_in = datetime.strptime('00:00', '%H:%M')
						diff = time_diff_in_hours(in_t,shift_in)
					elif pr.shift == 'G':
						shift_in = datetime.strptime('08:00', '%H:%M')
						diff = time_diff_in_hours(in_t,shift_in)
					elif pr.shift == 'G1':
						shift_in = datetime.strptime('09:00', '%H:%M')
						diff = time_diff_in_hours(in_t,shift_in)
					...
					if pr.shift == 'A':
						shift_out = datetime.strptime('16:00', '%H:%M')
						diff = time_diff_in_hours(shift_out,out_t)
					elif pr.shift == 'B':
						shift_out = datetime.strptime('00:00', '%H:%M')
						diff = time_diff_in_hours(shift_out,out_t)
					elif pr.shift == 'C':
						shift_out = datetime.strptime('08:00', '%H:%M')
						diff = time_diff_in_hours(shift_out,out_t)
					elif pr.shift == 'G':
						shift_out = datetime.strptime('17:00', '%H:%M')
						diff = time_diff_in_hours(shift_out,out_t)
					elif pr.shift == 'G1':
						shift_out = datetime.strptime('17:30', '%H:%M')
						diff = time_diff_in_hours(shift_out,out_t)
						...
						shift_out = datetime.strptime('04:00', '%H:%M')
						diff = time_diff_in_hours(shift_out,out_t)
					if diff <= 2:
						status = 'Present'
					else:
